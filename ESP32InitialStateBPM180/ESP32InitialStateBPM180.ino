...
#define bucketKey "KCAVBNE8TRFQ" // Bucket key (hidden reference to your bucket that allows appending):
#define bucketName "esp32" // Bucket name (name your data will be associated with in Initial State):
#define accessKey "5epLyaDHUFTP4NKbAsBtEoFIDBiu69ux" // Access key (the one you find in your account settings):

const int NUM_SIGNALS = 2; // How many signals are in your stream? You can have as few or as many as you want
String signalName[NUM_SIGNALS] = {"Temperature", "Pressure"}; // What are the names of your signals (i.e. "Temperature", "Humidity", etc.)
...
char ssid[] = "Halley_LAB"; // your network SSID (name)
char password[] = "H@LLEY2o13"; // your network password
...
signalData[0] = String(bmp.readTemperature());
signalData[1] = String(bmp.readPressure()/100.0);
...
if (client.connect(ISDestURL, 80) > 0) {
  ...
  // Build HTTP request.
  String toSend = "POST /api/buckets HTTPS/1.1\r\n";
  toSend += "Host:";
  toSend += ISDestURL;
  toSend += "\r\n" ;
